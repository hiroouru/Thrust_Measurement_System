...
// Define pins 
const int RELAY_PIN1 = 12; // For command 5
const int RELAY_PIN2 = 13; // For other 4
int command;
...
class AlarmCallbackHandler : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (value == "5") {
      ...
      command = 6;
      Serial.println("Triggered RELAY_PIN2 for command 6");
    }
  }
};
...
void setup() {
  ...
  BLEDevice::init("ESP32_Relay_Node");
  ...
  // Check for incoming commands from the client and relay them to the server
    ...
    if (command == 5) {
      ...
      command = 0;
        ...
        Serial.println("Sent restart mesurement command to server");
